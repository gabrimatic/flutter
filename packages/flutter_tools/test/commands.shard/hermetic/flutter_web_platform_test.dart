...
void main() {
  ...
  testWithoutContext('BrowserManager reports an unexpected browser disconnect', () async {
    // Regression test for the silent-teardown path: when the browser hangs up
    // while the tool still expects it, every unfinished suite is reported as
    // "did not complete" and, before this message existed, nothing said why.
    final chromiumLauncher = ChromiumLauncher(
      fileSystem: fileSystem,
      platform: platform,
      processManager: processManager,
    ...
    BrowserManager.forTesting(chromium, Runtime.chrome, FakeWebSocketChannel(controller), logger);
